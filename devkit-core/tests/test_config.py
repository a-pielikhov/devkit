...
def test_get_returns_none_when_no_config(tmp_path):
...
def test_add_creates_key(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    assert store.get("git", "protected_branches") == "main"


def test_add_appends_second_value_as_list(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_add_does_not_duplicate_existing_value(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    store.add("git", "protected_branches", "main")
    assert store.get("git", "protected_branches") == "main"
...
def test_add_replace_overwrites(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    store.add("git", "protected_branches", "develop")
...
def test_add_multiple_modules_isolated(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_remove_specific_value_from_list(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    store.add("git", "protected_branches", "develop")
...
def test_remove_last_value_deletes_key(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_remove_entire_key(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_remove_missing_key_is_silent(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.remove("git", "nonexistent_key")


def test_remove_missing_value_is_silent(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_remove_all_clears_module(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
...
def test_remove_all_leaves_other_modules(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    store.add("net", "timeout", "30")
    store.remove_all("git")
...
def test_show_returns_module_dict(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    store.add("git", "protected_branches", "main")
    store.add("git", "default_branch", "main")
...
def test_show_returns_empty_dict_when_missing(tmp_path):
    store = ConfigStore(tmp_path / "config.toml")
    assert store.show("git") == {}
